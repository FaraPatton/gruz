// ══ PDF Generation — кириллица через Roboto ══════════════════════════

let _fontLoaded = false;
let _fontBoldLoaded = false;

async function ensureFont(doc) {
  if (!_fontLoaded) {
    try {
      const r = await fetch('https://fonts.gstatic.com/s/roboto/v32/KFOmCnqEu92Fr1Me5WZLCzYlKw.ttf');
  ...
  if (!_fontBoldLoaded) {
    try {
      const r = await fetch('https://fonts.gstatic.com/s/roboto/v32/KFOlCnqEu92Fr1MmWUlfBBc9.ttf');
      const buf = await r.arrayBuffer();
      const bytes = new Uint8Array(buf);
      let bin = '';
      for (let i = 0; i < bytes.length; i += 8192)
        bin += String.fromCharCode.apply(null, bytes.subarray(i, i + 8192));
      doc.addFileToVFS('Roboto-Bold.ttf', btoa(bin));
      doc.addFont('Roboto-Bold.ttf', 'Roboto', 'bold');
      _fontBoldLoaded = true;
    } catch(e) { console.warn('Font bold load failed:', e); }
  }
...
function sf(doc, s, b) {
  try { doc.setFont('Roboto', b ? 'bold' : 'normal'); }
  catch(e) { doc.setFont('helvetica', b ? 'bold' : 'normal'); }
  doc.setFontSize(s);
}
