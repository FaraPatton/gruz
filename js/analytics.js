...
function usnTaxFromYearStart(trips) {
  const currentYear = new Date().getFullYear();
  return (trips || [])
    .filter(trip => Number(trip.year) === currentYear)
    .reduce((sum, trip) => sum + usnTax(trip), 0);
}
...
function renderDriveAnalytics(entries, yr, panel) {
  ...
  const usnTaxYtd = usnTaxFromYearStart(trips);
  ...
  const overviewHtml =
    ...
    '<div class="dash-mini-grid">' +
      ...
      dashboardMetricCard('6%', money(usnTaxYtd), 'налоги', 'УСН с начала года') +
    '</div>' +
