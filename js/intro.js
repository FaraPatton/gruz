...
(() => {
  ...
  Promise.all(images.map(img => img.decode())).then(() => {
    ...
    dialog.innerHTML = '<div class="gruz-intro__scene"></div><div class="gruz-intro__scene gruz-intro__scene--end"></div><div class="gruz-intro__shade"></div><div class="gruz-intro__top"><span>GRUZ / МОСКВА</span><button class="gruz-intro__skip" autofocus>К работе →</button></div><div class="gruz-intro__copy"><p class="gruz-intro__eyebrow">КАЖДЫЙ РЕЙС ПОД КОНТРОЛЕМ</p><h2>Всё готово.<br>Можно в путь.</h2><p class="gruz-intro__caption">ИП Карпов С.В. · Грузоперевозки</p></div><div class="gruz-intro__line"></div>';
