...
- Авторизованный браузер получает реквизиты для клиентской генерации PDF, но не получает `ARCHIVE_ROOT`.
...
- Google access token убран из браузерного хранения на основном Vercel-домене.

### Низкий приоритет

- API key Яндекс Карт может быть публичным только при ограничении по доменам и квотам.
...
### Этап 7.1. Server-side OAuth session

Статус: выполнено для Vercel-домена. Добавлен Google OAuth redirect flow:
