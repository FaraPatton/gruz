...
- Реквизиты, Drive ID, печать и настройки аналитики выдаются защищенным Vercel endpoint только после whitelist.
...
- `trips.json` и Gmail перенесены на Vercel backend. Архивирование PDF, пересборка реестра и печать пока работают через браузер.

## Риск-профиль

### Высокий приоритет

- Авторизованный браузер пока получает реквизиты и Drive ID для клиентской генерации PDF и пересборки архива.
